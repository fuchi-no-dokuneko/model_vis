...
from test.e2e.audit_helpers import ARTIFACTS, open_model, rect, screenshot, settle, visible_click
...
def test_audit_52_viewports_have_reachable_navigation(driver, viewer_url, model):
    ...
    for checkbox in driver.find_elements(By.CSS_SELECTOR, ".compare-check input")[:2]:
        checkbox.click()
    outcomes=[]
    for width,height in SIZES:
        ...
        outcomes.append(dict(model=model,width=width,height=height,navigation=True,inspector=panel,document_width=width))
